...
`a2a_message` may request OMP UI approval for one exact outbound Message. The Agent proposing the approval-gated action owns that request at its sending endpoint; a receiving Agent never asks its local user to approve on the sender's behalf. For an unsigned approval-gated request, the receiver tells the sender to preserve the target selector, text, causal parent, and attachments but use a new `messageId` and request approval at its own endpoint; reusing the unsigned ID would conflict because the receipt changes persisted Message content. OMP's official, collaboration-aware `askDialog` provides the scrollable review; a connected collaboration Guest may answer it on behalf of the sending endpoint. The visible review is a human-readable Target/Message/Attachments layout containing only the target, text, and attachment names and source URLs. Older OMP releases without `askDialog` fall back to `confirm`/`input`, and missing interactive UI fails closed before attachment snapshotting. The Extension separately binds approval to the sending Presence, Project, exact `messageId`, target selector, text, causal parent, and snapshotted attachment names and bytes, with Session and connection fences preventing changes before dispatch. The canonical Message persists the resulting non-cryptographic wire receipt. Realtime and history context exposes its derived model-facing state as `senderUserApproval: "confirmed" | "unsigned"`, with peer text JSON-escaped behind an unambiguous record boundary. Approval does not propagate through replies, forwarding, or delegation; rejection sends nothing and may return an exact user reason; cancellation is not cached.
