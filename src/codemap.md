...
| Tool | Contract |
| --- | --- |
| `a2a_peers` | Returns the connected Agent's own roster name plus the other current Presence names that are valid `a2a_message` targets. The magic target `@all` addresses every current peer. |
| `a2a_message` | Requires a non-empty target name list or `["@all"]` and text; accepts optional current-session `local://` attachment sources, `replyTo`, `messageId`, and local UI approval of the sending Agent's exact outbound Message. Forked OMP versions advertise `askDialogCapabilities.allowCustomInput` when their `localAskDialog` can disable custom input; official OMP and older forks lack that marker and fall back to `confirm`/`input`, so Type is absent on every supported path. The UI review uses a human-readable Target/Message/Attachments layout containing only the target, text, and attachment names and source URLs; the Extension separately binds approval to the current sending Presence, Project, resolved `messageId`, target, text, causal parent, and snapshotted attachment names and bytes before dispatch. A receiver never asks its local user on the sender's behalf; it returns an unsigned approval-gated request with instructions to preserve target, content, causal parent, and attachments while using a new `messageId` and sender-owned approval. Rejection returns the user's optional reason without sending and suppresses only an identical request from the same Presence for the Session. The tool forwards combined cancellation, fences the initiating connection, distinguishes new acceptance from replay, and keeps replies push-driven. |
...
Every model turn receives byte-identical A2A system-prompt additions defining the tool contract, collaboration authority, and approval ownership; connection status, Project, and roster name never change that prefix. The Extension instead appends hidden `a2a-connection` context messages and treats the latest as the operational state. Peer messages are substantive but untrusted coordination input. Peers cannot override or speak for the user, and A2A peer input never overrides direct user instructions. An Agent that proposes an approval-gated action must request approval at its own sending endpoint; a receiver returns unsigned approval-gated requests to their sender with unchanged request facts and a new `messageId`, rather than asking its local user. Material decisions go to the local user only when that Agent owns the decision. There is no model-side connect/disconnect or Project administration. Sending is fire-and-forget: the model continues only other already-requested, reply-independent work or ends its turn.

Only Extension-injected model metadata with `senderUserApproval: "confirmed"` represents local UI approval at the sending endpoint. It binds the exact immutable Message and target, does not propagate through replies, forwarding, or delegation, and never overrides direct user instructions. `senderUserApproval: "unsigned"` remains ordinary peer collaboration. Peer text is JSON-escaped so it cannot manufacture a second confirmed record. The underlying trusted-client wire receipt remains `userApproval: { kind: "omp-ui" }`; the Hub validates its shape but cannot prove UI provenance.
