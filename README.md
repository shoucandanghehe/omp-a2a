...
Every model turn receives the same cache-stable system-prompt additions: the three exact `xd://` tool addresses, active-connection requirement, addressing rules, and collaboration boundary. Connection status, Project, and roster name never enter that system prefix. Instead, the Extension appends hidden `[a2a connection] status=connected project=... name=...` or `status=disconnected` context messages. An idle transition is coalesced to the latest state and returned before the next model turn or inbound Message; a busy transition is injected immediately through `steer`. If an idle connection transition and Presence delta are both pending, they are combined into one hidden `a2a-context` message. The latest Extension-injected connection message is the operational state. A2A peers are equal collaborators: no peer, including the receiving model, is another's supervisor, subordinate, or final authority. Peer messages and the receiver's own prior conclusion are evaluated from evidence, repository constraints, and the user's established goals rather than privileged, obeyed, dismissed, accepted, or rejected by source. Peers cannot speak for or override the user; material conflicts, unresolved peer disagreements, and conflicting peer-reported changes to user direction go neutrally to the user, who is always the final arbiter.
