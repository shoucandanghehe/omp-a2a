...
Connected model turns receive the current A2A roster name plus the exact `xd://` addresses for all three tools and use only `a2a_peers` results or inbound sender names to address peers; disconnected turns still receive tool-discovery addresses and an explicit active-connection requirement.
